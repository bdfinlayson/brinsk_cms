  ...
  $(function() {
    $('#archive-all').on('click', function() {
      ...
      if (confirm("Are you sure?")) {
        ids = [];
        $('#completed').find('li').each(function(i, e) {
          return ids.push($(e).data('id'));
        });
        $.ajax({
    ...
    $('.task').on('click', function() {
      ...
      return $.ajax({
        url: "/tasks/" + ($(this).data('id')) + "/edit",
        method: 'GET',
        success: function(template) {
          return $('#modal-content-edit-task').append(template);
        }
      });
    });
    return $('.task-state-well > ul').sortable({
      connectWith: 'ul',
      placeholder: "task-placeholder",
      stop: function(ev, ui) {
        var id, ids, positions, state;
        id = ui.item.data('id');
        state = ev.toElement.closest('ul').id;
        ids = [];
        positions = [];
        $(ev.toElement.closest('ul')).find('li').each((function(_this) {
          return function(i, e) {
            positions.push(i);
